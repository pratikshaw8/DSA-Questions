...
Node* solve(int pre[],int size, int &i, int min, int max)
{
    if(i>=size) return NULL;
    if(pre[i] <min or pre[i]>max)
    return NULL;
    
    Node* root=newNode(pre[i]);
    i++;
    root->left=solve(pre,size,i,min,root->data);
    root->right=solve(pre,size,i,root->data,max);
    return root;
}
...
Node* post_order(int pre[], int size)
{
    ...
    int i=0;
    return solve(pre,size,i,min,max);
